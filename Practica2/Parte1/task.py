...
# Evitar warnings extraños en algunos entornos
os.environ["HF_HUB_DISABLE_SYMLINKS_WARNING"] = "1"

# Cachés globales para datasets y particiones
_train_dataset = None
_test_dataset = None
...
def create_partitions(num_partitions: int, min_classes_per_client: int = 2, max_classes_per_client: int = 3):
    """
    Crea un particionado NO IID:
      - Cada cliente ve solo 2 o 3 clases.
      - Se asegura que todas las clases aparezcan al menos en un cliente.
    """
    global _partitions
    ...
    # 2) Asegurar que todas las clases aparezcan
...
# =========================================================
class MLP(nn.Module):
    """Modelo MLP simple para Fashion-MNIST"""
    def __init__(self):
        super().__init__()
...
class SmallCNN(nn.Module):
    """CNN pequeña opcional para Fashion-MNIST"""
    def __init__(self):
        super().__init__()
...
def create_model(model_type: str):
    """Crea modelo PyTorch según tipo ('mlp' o 'cnn')"""
    if model_type == "mlp":
        return MLP()
    elif model_type == "cnn":
        return SmallCNN()
...
# =========================================================
# FUNCIONES DE AYUDA PARA FEDERATED LEARNING
# =========================================================
def get_model_parameters(model: nn.Module) -> NDArrays:
    """Convierte state_dict de PyTorch a lista de arrays para Flower"""
...
def set_model_params(model: nn.Module, params: NDArrays) -> nn.Module:
    """Carga parámetros (lista de arrays) en un modelo PyTorch"""
...
def train_one_round(model: nn.Module, train_loader: DataLoader, device: torch.device,
    ...
    if proximal_mu > 0.0 and global_params is not None:
        # Convertir parámetros globales a tensores para término proximal
    ...
    for _ in range(epochs):
        for data, target in train_loader:
            ...
            if proximal_mu > 0.0 and global_tensors is not None:
                prox = sum(torch.sum((w - w0)**2) for w, w0 in zip(model.state_dict().values(), global_tensors))
                loss += (proximal_mu / 2.0) * prox
            loss.backward()
            optimizer.step()

def test(model: nn.Module, data_loader: DataLoader, device: torch.device) -> Tuple[float, float]:
    """Evalúa modelo: devuelve (loss_media, accuracy)"""
    model.to(device)
    model.eval()
    criterion = nn.CrossEntropyLoss()
...
# =========================================================
# GENERACIÓN DE HISTOGRAMAS POR CLIENTE
# =========================================================
if __name__ == "__main__":
    """
    Ejecuta:
        python task.py
    Se generarán hist_client_0.png, hist_client_1.png, ..., uno por cliente.
    """
